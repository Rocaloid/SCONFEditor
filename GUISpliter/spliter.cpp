...
{
    QString QSCONFPatht = QFileDialog::getOpenFileName(this,tr("打开SCONF"), QDir::currentPath(), tr("分割配置文件(*.sconf)(*.sconf)"));
...
{
    QString QWavePatht = QFileDialog::getOpenFileName(this,tr("打开WAVE"), QDir::currentPath(), tr("波形文件(*.wav)(*.wav)"));
...
void Spliter::on_action_SCONF_2_triggered()
{
    if(this->SCONFLoaded)
    {
        QString qpath = QFileDialog::getSaveFileName( this, tr("保存SCONF"), QDir::currentPath(), tr("分割配置文件(*.sconf)(*.sconf)") );
